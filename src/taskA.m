...
hyp = struct('mean', [], 'cov', [-1, 0], 'lik', 0); % set parameters arbitrarily
...
hold on;
name = "Trained Gaussian Process for cw1a.mat data";
title({
    name 
    sprintf("initial values: hyp.cov=[%.2f, %.2f], hyp.lik = %.2f", hyp.cov(1), hyp.cov(2), hyp.lik)
    sprintf("optimised values: hyp.cov=[%.2f, %.2f], hyp.lik = %.2f", optHyp.cov(1), optHyp.cov(2), optHyp.lik)
});
